...
// gfx942 (CDNA3): FNUZ FP8, wave64
// gfx950 (CDNA4): OCP  FP8, wave64  <-- rocwmma ROCWMMA_FP8 supports gfx950 with fp8_e4m3
// gfx11xx/gfx12xx (RDNA): OCP FP8, wave32
#ifdef __ROCM_ARCH_GFX942
  using fp8_type = __hip_fp8_e4m3_fnuz;
  namespace Params = gfx9Params;
#elif defined(__ROCM_ARCH_GFX950)
  using fp8_type = __hip_fp8_e4m3;    // OCP FP8 (ROCWMMA_FP8 path, wave64)
  namespace Params = gfx9Params;      // CDNA4 = wave64, same tile params as CDNA3
#else
            ...
            DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
                DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
                    DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
                        ...
                        // Use original (unpadded) M and N for scale shape checks,
                        // since quantization scales were computed from the unpadded tensors.
                        if constexpr (QK_QUANT_GRAN == static_cast<int>(QuantGranularity::kPerWarp))
                        {
                          CHECK_SHAPE(query_scale, batch_size, num_qo_heads, div_ceil(M, CTA_Q) * (CTA_Q / WARP_Q));
                          CHECK_SHAPE(key_scale, batch_size, num_kv_heads, div_ceil(N, CTA_K) * (CTA_K / WARP_K));
                        }
                        else if constexpr (QK_QUANT_GRAN == static_cast<int>(QuantGranularity::kPerThread))
                        {
                          CHECK_SHAPE(query_scale, batch_size, num_qo_heads, div_ceil(M, CTA_Q) * (CTA_Q / WARP_Q) * 8);
                          CHECK_SHAPE(key_scale, batch_size, num_kv_heads, div_ceil(N, CTA_K) * (CTA_K / WARP_K) * 4);
                        }
                        else
                        {
