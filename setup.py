...
if IS_ROCM:
    ...
    if rocm_arch:
        ...
        # rocwmma 7.x has a static_cast<__half>(0.0f) in its HIP_vector_base macro that
        # fails when PyTorch sets __HIP_NO_HALF_CONVERSIONS__=1.
        # We undefine that macro for the hipcc compilation unit so rocwmma compiles cleanly.
        rocm_hipcc = base_flags + debug_flags + [f"--offload-arch={rocm_arch}"] + [f"-D__ROCM_ARCH_{rocm_arch.upper()}"] + ["-U__HIP_NO_HALF_CONVERSIONS__", "-U__HIP_NO_HALF_OPERATORS__"]
